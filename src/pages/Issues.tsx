...
import {
  AlertTriangle, Search, CheckCircle, Trash2, Edit2, Calendar, User, Info,
} from 'lucide-react';
...
function Issues() {
  ...
  const handleConfirmAction = async () => {
    ...
    try {
      ...
      setConfirmModal((prev) => ({
        ...prev, isOpen: false, issueId: null, type: null,
      }));
      refetch();
    } catch (err) {
  ...
  const filteredIssues = issues.filter((issue) => issue.issueItem.toLowerCase().includes(searchTerm.toLowerCase())
            ...
            <div className="space-y-6">
              <div>
                ...
                <p className="text-sm text-gray-500 mt-1">
                  MDA:
                  {(selectedIssue as any).project?.mda?.name || 'N/A'}
                </p>
              </div>
